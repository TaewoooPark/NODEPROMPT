...
/**
 * N=50, D=5 계층적 데모 데이터.
 * "AI가 인간 사회에 미치는 다층적 영향"을 주제로 한 개념 트리.
 */
...
const DEMO_EDGES: { sourceId: string; targetId: string; relation: string; strength: number }[] = [
  // 횡단 연결 (cross-link)
  ...
  // parent-child (트리 구조 — 자동 생성분 외 명시적)
  ...DEMO_NODES.filter(n => n.parentId).map(n => ({
    sourceId: n.parentId!, targetId: n.id, relation: 'parent-child' as const, strength: 0.8,
  })),
];

export function loadDemoData(sphereRadius: number): { nodes: NodeData[]; edges: EdgeData[] } {
  return mapHierarchicalToSphere(DEMO_NODES, DEMO_EDGES, sphereRadius, 5);
}
